...
public class RandomMeteorsTwo extends GameObject{
	...
	public RandomMeteorsTwo(float x, float y, Handler handler, ObjectId id) {
		super(x, y, id);
		this.handler = handler;
		
		bigMeteorFlying = new Animation(20, tex.MeteorFlying[0], tex.MeteorFlying[1], tex.MeteorFlying[2], tex.MeteorFlying[3]);
	...
	public void tick(LinkedList<GameObject> object) {
		...
		if(BossEnemyShip.waitForMoonExploding >= 16000){
			velY = 1;
			velX = -2;
		}
	...
	public void collision(LinkedList<GameObject> object){
		...
		for(int i = 0; i < handler.object.size(); i++){
			GameObject tempObject = handler.object.get(i);
			if(tempObject.getId() == ObjectId.RandomMeteorsTwo){
