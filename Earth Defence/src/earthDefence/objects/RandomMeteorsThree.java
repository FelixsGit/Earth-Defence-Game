...
public class RandomMeteorsThree extends GameObject{
	...
	public RandomMeteorsThree(float x, float y, Handler handler, ObjectId id) {
		super(x, y, id);
		this.handler = handler;
		
		bigMeteorFlying = new Animation(10, tex.MeteorFlying[0], tex.MeteorFlying[1], tex.MeteorFlying[2], tex.MeteorFlying[3]);
	...
	public void tick(LinkedList<GameObject> object) {
		...
		if(BossEnemyShip.waitForMoonExploding >= 18000){
			velY = 2;
			velX = -2;
		}
	...
	public void collision(LinkedList<GameObject> object){
		...
		for(int i = 0; i < handler.object.size(); i++){
			GameObject tempObject = handler.object.get(i);
			if(tempObject.getId() == ObjectId.RandomMeteorsThree){
